...
public class SysRoleModuleBiz {
	...
	public int addRoleModules(int id, String tempId) {
		...
		//根据 角色id查询是否 此角色下面是否已经存在 权限   先删除对应点 存在的角色权限关系
		/*List<SysRoleModule> list = sysRoleModuleMapper.selectRoleModuleListByMap(map);
		if(null != list && list.size()>0){
			
		}*/
		//删除已经的记录
		int t = sysRoleModuleMapper.deleteRoleModuleByRoleId(id);
		//循环插入角色权限关系
		int returnValue = 0 ;
		for(String s : ids){
